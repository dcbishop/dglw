#ifndef DGLW_BUFFER_HPP_
#define DGLW_BUFFER_HPP_
...
class Buffer {
   public:
      ...
      void bindRange(const GLuint& index, const GLintptr& offset, const GLsizeiptr& size);
      ...
      void data(const std::vector<GLfloat>& data, const Usage usage=StaticDraw);
      void subData(const Target& target, const GLintptr& offset, const GLsizeiptr& size, const GLvoid* data);
...
inline void Buffer::data(const std::vector<GLfloat>& data, const Usage usage) {
   bufferData_(target_, data.size() * sizeof(GLfloat), &data[0], usage);
}

inline void Buffer::subData(const Target& target, const GLintptr& offset, const GLsizeiptr& size, const GLvoid* data) {
   bind(target);
...
inline void Buffer::subData(const GLintptr& offset, const GLsizeiptr& size, const GLvoid* data) {
...
inline void Buffer::bindRange(const GLuint& index, const GLintptr& offset, const GLsizeiptr& size) {
   bind();
